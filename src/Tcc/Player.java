...
public class Player extends Application {
    ...
    public Player(){
        ...
        caminhoVideo = TCC.caminho = TCC.caminho.replaceAll("[\\\\]", "/");
        TCC.caminho = TCC.caminho.replaceAll("[ ]", "%20");
        TCC.caminho = TCC.caminho.replace("[", "%5B");
        TCC.caminho = TCC.caminho.replace("]", "%5D");
        caminhoVideo = "file:/"+TCC.caminho;
       ...
       //Ação do botão Fullscreen
        ...
        sim.setOnAction(new EventHandler<ActionEvent>() {  
            ...
            public void handle(ActionEvent event1) {  
              ...
              TCC.caminho = caminhoVideo;
              new Imagens();
              /*
              //Criando o painel das imagens e a caixa que exibirá a imagem
              Imagens.stack = new StackPane();
              VBox atalho = new VBox();
              atalho.setLayoutY((double)TCC.lay);
              HBox box = new HBox();
              ...
              //Tirando o frame e adicionando as propriedades aos seus respectivos lugares
              Imagens.imagem = view.snapshot(new SnapshotParameters(), new WritableImage(w,h));
              ImageView shot = new ImageView();
              shot.setImage(Imagens.imagem);
              Image video = new Image(getClass().getResourceAsStream("Video.gif"));
              ...
              //Botão de video, criando e adicionado no StackPane
              but = new Button();
              but.setGraphic(new ImageView(video));
              but.setMaxSize(100,100);
              
              //Adicionando o botão no Stack, o Stack no painel e propriedades de layout
              Imagens.stack.getChildren().addAll(but);
              atalho.getChildren().add(Imagens.stack);
              TCC.cont++;
              if(TCC.cont>=1){
                TCC.lay+=110;
                Imagens.stack.setLayoutY(TCC.lay);
              }
              
              
             //botão de sair do frame no programa principal
             ...
             @Override  
              public void handle(ActionEvent event1) {  
                    principal.getChildren().removeAll(box,atalho);
                    TCC.cont--;
                    TCC.lay-=110;
                }
            });
            ...
            //Ação do botão do display quando clicado
            ...
            public void handle(ActionEvent event2) {  
                ...
                //Adicioando a imagem e o botão sair na caixa e depois adicionando a caixa e o painel no programa principal
                box.getChildren().addAll(shot, sp, sair);
                principal.getChildren().addAll(box, atalho);*/
            }
        });
